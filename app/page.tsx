            ...
            <CarouselContent>
              {[
                ...
                <CarouselItem key={index}>
                  <div className="relative h-44 sm:h-52 md:h-60 lg:h-64 rounded-3xl overflow-hidden flex items-center">
                    <img
                      src={slide.image}
                      alt={slide.title}
            ...
            <div className="h-[600px] overflow-y-auto pr-2">
              <div className="grid gap-3 sm:gap-6 grid-cols-3 sm:grid-cols-4 lg:grid-cols-3">
                {filteredItems.map((item, idx) => (
                  <div
                    key={item._id}
                    className="relative bg-white rounded-xl sm:rounded-2xl p-2 sm:p-4 shadow-sm flex flex-col"
                  >
                    <span className="absolute top-2 right-2 text-[9px] sm:text-[10px] bg-black/80 text-white px-1.5 py-0.5 rounded-full">
                      Popular
                    </span>
                    ...
                    <Button
                      ...
                      className="mt-auto w-full py-1.5 sm:py-2 rounded-lg sm:rounded-xl text-xs sm:text-sm font-extrabold border border-[#0C831F] text-[#0C831F] hover:bg-[#0C831F] hover:text-white transition"
                    >
                      ADD
                    </Button>
                  </div>
    ...
    {showMobileFilter && (
      <div className="fixed inset-0 z-50 bg-black/40 lg:hidden">
        <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-3xl p-5 max-h-[85vh] overflow-y-auto">
            ...
            {categories.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`border rounded-xl px-3 py-2 text-sm font-semibold transition ${
                    active
          ...
          <div className="mb-6">
            <p className="font-semibold text-sm mb-2">Max Price: ₹{priceRange}</p>
            <input
              type="range"
              min={50}
              max={2000}
